...
CREATE TABLE `prod_engagements` (
  ...
  `status` enum('en attente','prévu','reporté','fait','annulé','retour client') NOT NULL DEFAULT 'en attente',
  `commentaire` text DEFAULT NULL,
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013  SQL SECURITY DEFINER */
/*!50001 VIEW `vue_prod_planning` AS select `pp`.`id` AS `id`,`pa`.`reference` AS `reference`,`pa`.`couleur` AS `reference_couleur`,`ptse`.`designation` AS `type`,`pp`.`numero` AS `numero`,`pp`.`semaine` AS `semaine`,`pp`.`prete` AS `prete`,case when `pe`.`status` = 'fait' then 'Fait' when `pe`.`status` = 'annulé' then 'Annulé' when `pe`.`status` = 'reporté' then 'Reporté' when `pe`.`status` = 'prévu' then 'Prévu' else 'En Attente' end AS `status`,`pp`.`fk_article` AS `fk_article`,`pp`.`fk_type_sous_ensemble` AS `fk_type_sous_ensemble` from (((`prod_planning` `pp` join `prod_articles` `pa` on(`pp`.`fk_article` = `pa`.`id`)) join `prod_type_sous_ensemble` `ptse` on(`pp`.`fk_type_sous_ensemble` = `ptse`.`id`)) left join (select `pe1`.`id` AS `id`,`pe1`.`fk_planning` AS `fk_planning`,`pe1`.`semaine_engagee` AS `semaine_engagee`,`pe1`.`status` AS `status`,`pe1`.`commentaire` AS `commentaire`,`pe1`.`modified_at` AS `modified_at` from (`prod_engagements` `pe1` join (select `prod_engagements`.`fk_planning` AS `fk_planning`,max(`prod_engagements`.`modified_at`) AS `max_created` from `prod_engagements` group by `prod_engagements`.`fk_planning`) `latest` on(`pe1`.`fk_planning` = `latest`.`fk_planning` and `pe1`.`modified_at` = `latest`.`max_created`))) `pe` on(`pe`.`fk_planning` = `pp`.`id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-05-05  8:37:45
